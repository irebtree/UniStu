...
using System.IO;
/// <summary>
/// assetBundles Build script.
/// by.xu
/// </summary>
public class BuildScript
{
	const string kAssetBundlesOutputPath = "AssetBundles";
	const string kAssetBundlesOutputPathUnCompreb = "AssetBundles/Uncopre";
	static string platform = BundleManager.GetPlatform();
	public static void BuildAssetBundles()
	{
		// Choose the output path according to the build target.
		string path = EditorUtility.SaveFolderPanel ("Save Bundle", EditorPrefs.GetString("BundleManagerLastDir", ""), "");
		string outputPath = Path.Combine(kAssetBundlesOutputPath,   platform );
		outputPath = Path.Combine(path,platform);
	...
	public static void BuildAssetBundlesUncompressed()
	{
		// Choose the output path according to the build target.
		string outputPath = Path.Combine(kAssetBundlesOutputPathUnCompreb, platform );
		if (!Directory.Exists(outputPath) )
			Directory.CreateDirectory (outputPath);
